...
and then report instances of alternative usages as violations.  In
this work, we introduce a learning-based approach for API misuse
detection that avoids the need of pre-defined thresholds. To overcome
the lack of training data, we leverage two key data augmentation
techniques to apply for AUG, a graph-based representation of API
usages. We also leverage Labeled, Graph-based Convolutional Network to
learn the embeddings for API usages, which capture the key features in
API usages and be used for API misuse detection. Our empirical
evaluation on real-world, API misuse benchmark shows that our model
can relatively improve over the state-of-the-art API misuse detection
from 30--35\%.
\end{abstract}
